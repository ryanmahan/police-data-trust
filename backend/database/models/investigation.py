...
class Investigation(db.Model):
  ...
  finding = db.Column(db.Enum(Finding)) # Did this happen? Exoneration? (Status or invstigation result; enum with standardized options)
  # recomendation: # Might be useful to differentiate invstigator outcome and final outcome
  # TODO: Also an enum?
  outcome = db.Column(db.String) # Outcomes may change based on the insitution; Can be overriden downstream, 
  # Reason for Outcome?
  # TODO: also an enum?
  ...
  jurisdiction =  db.Column(db.String)# Should we have a jurisdiction table? Allegation Tables?